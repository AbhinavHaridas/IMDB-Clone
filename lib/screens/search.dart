...
class TopicSearch extends SearchDelegate<String> {
  ...
  Future<List> fetchTypeOfSearch(suggestion) async {
        ...
        if (response.statusCode == 200) {
          ...
          print(resultsJson.runtimeType);
          final type = [];
           type.add("${resultsJson["Type"]}" " " "${resultsJson["Year"].toString()}");
              ...
              builder: (context, snapshot) {
                if (snapshot.hasData) {
                  ...
                  return SizedBox(
                      ...
                      child: GestureDetector(
                          onTap: () {
                            Navigator.pushNamed(context, '/mediainfo', arguments:{"name": suggestion} );
                          },
                          child: ListTile(
                            leading: Image.network(type![1]),
                            title: Text(suggestion, style: const TextStyle(color: Colors.white)),
                            subtitle: Text(type[0], style: const TextStyle(color: Colors.grey))
                        )),
                      );
                } else {
                  return ListTile(
      ...
      children: [
        ColorFiltered(
          ...
          child: Container(
            ...
            child: Image(width: 200, height: 150, image: NetworkImage(images[0])),
          ),
        ),
        ColorFiltered(
          colorFilter:
          ...
          child: Container(
            ...
            child: Image(width: 200, height: 150, image: NetworkImage(images[1])),
          ),
        ),
        Container(
